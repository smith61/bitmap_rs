...
impl<S: AsRef<[B]> + ?Sized, B: BitStore> Bitmap<S, B> {
    ...
    pub fn as_slice<'a>(&'a self) -> BitmapSlice<'a, B> {
        ...
        if buffer.len() > Self::MAXIMUM_BUFFER_SIZE {
            panic!(
                "Bitmap buffer is too large ({} > {})",
                buffer.len(),
                Self::MAXIMUM_BUFFER_SIZE
            );
        }

        unsafe { BitmapSlice::new_unchecked(buffer, 0, buffer.len() * B::BIT_COUNT) }
    }
...
impl<S: AsRef<[B]> + AsMut<[B]> + ?Sized, B: BitStore> Bitmap<S, B> {
    ...
    pub fn as_slice_mut<'a>(&'a mut self) -> BitmapSliceMut<'a, B> {
        ...
        unsafe { BitmapSliceMut::new_unchecked(buffer, 0, buffer.len() * B::BIT_COUNT) }
    }
