...
impl<'a, B: BitStore, M: Mutability> BitmapSliceImpl<'a, B, M> {
    pub(super) fn find_next_in_range<const CLEAR_BIT: bool>(
        &self,
        range: Range<usize>,
    ) -> Option<usize> {
        if range.is_empty() {
            ...
            if (range.end - range.start) > total_bit_count {
                panic!(
                    "Invalid bit range [{}:{}] for bitmap of size {}",
                    range.start, range.end, total_bit_count
                );
            }
        }
...
impl<'a, B: BitStore> BitmapSliceImpl<'a, B, Mut> {
    ...
    pub(super) fn modify_bit(&mut self, bit_index: usize, operation: BitmapSliceOperation) {
        ...
        unsafe {
            operation.apply(
                self.buffer_address.as_ptr().add(slot),
                B::create_bit_mask(offset),
            );
        }
    }

    #[inline(always)]
    pub(super) fn modify_bit_range(
        &mut self,
        bit_range: Range<usize>,
        operation: BitmapSliceOperation,
    ) {
        if bit_range.is_empty() {
            return;
        }

        if (bit_range.start >= self.size()) || (bit_range.end > self.size()) {
            panic!(
                "Invalid bit range [{}:{}] for bitmap of size {}",
                bit_range.start,
                bit_range.end,
        ...
        if remaining != 0 {
            unsafe {
                operation.apply(buffer, B::create_range_mask(current_offset, remaining));
            }
        }
    }
}
